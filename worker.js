...
const fetchMeetingAttendees = async (meetingId) => {
  try {
    const response = await hubspotClient.apiRequest({
      method: "GET",
      path: `/crm/v3/associations/meetings/contacts`,
      params: {
        id: meetingId,
        limit: 100,
      },
    ...
    const associations = response.body?.results || [];
    return associations.map((assoc) => assoc.to.id);
  } catch (err) {
    console.error(`Failed to fetch attendees for meeting ${meetingId}`, err);
    return [];
  }
};
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    for (const meeting of data) {
      ...
      const attendees = await fetchMeetingAttendees(meetingId);
